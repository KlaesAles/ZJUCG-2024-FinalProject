...
Renderer::Renderer(GLFWwindow* win, unsigned int width, unsigned int height, Camera& cam,
    LightManager& lm, ShadowManager& sm, Scene& sc, CharacterMove &ch)
    : window(win), SCR_WIDTH(width), SCR_HEIGHT(height),
    camera(cam), lightManager(lm), shadowManager(sm), scene(sc), ch(ch),
...
void Renderer::processInput()
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS || 
        glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
    {
        if (!gPressed) {  // G 键触发 Running 动画
            gPressed = true;
        }
        if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) ch.setToward(glm::vec3(0.0f, 0.0f, -1.0f));
        if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) ch.setToward(glm::vec3(0.0f, 0.0f, 1.0f));
        if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) ch.setToward(glm::vec3(-1.0f, 0.0f, 0.0f));
        if(glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) ch.setToward(glm::vec3(1.0f, 0.0f, 0.0f));
    }
    else {
    ...
    ch.update(gPressed, jPressed);
    CharacterState state = ch.getState();
    if (state == CharacterState::Idle)  Character->playAnimation("Idle");
    if (state == CharacterState::Running) Character->playAnimation("Running");
    if (state == CharacterState::Jumping) Character->playAnimation("Jump");
    /*
